...
int BasicLs(HANDLE hConsoleOutput, CONSOLE_SCREEN_BUFFER_INFO lpConsoleScreenBufferInfo, int argc, WCHAR *argv[])
{
            ...
            if (wcslen(argv[index]) > 2) {
              ...
              if (!wcscmp(str_arg, L"all")) {
                ...
                puts(
                    ...
                    "Sort entries alphabetically if none of -rtS is specified.\n"
                    "\n"
                ...
                puts(
                    "pretty-ls-for-windows (sensod9/pretty-ls-for-windows) 1.0\n"
  ...
  if (hFind != INVALID_HANDLE_VALUE) {
    ...
    if ((wcscmp(FileName, L"..") != 0 || flag_a) && 
      ...
      if (!(FileAttributes & FILE_ATTRIBUTE_HIDDEN) || flag_A || flag_a) {
        ...
        FileInfoArray[index] = CurrentFileInfo;
        index++;
      }
    }
    ...
    printf("Fatal error occured: %lu.\nType \"ls --help\" for usage.", GetLastError());
    return (1);
  }
  ...
  if (index != 0) {
    if (flag_S)
      qsort(FileInfoArray, index, sizeof(FileInfo), fiCompBySize);
    else if (flag_t)
      qsort(FileInfoArray, index, sizeof(FileInfo), fiCompByTime);
    if (flag_r)
      ReverseFileInfoArray(index, FileInfoArray);

    if (flag_l)
      PrintLongListing(hConsoleOutput, lpConsoleScreenBufferInfo, CurrentDirectory, index, FileInfoArray);
    else
      PrintShortListing(hConsoleOutput, lpConsoleScreenBufferInfo, CurrentDirectory, index, FileInfoArray, flag_1);
  }
